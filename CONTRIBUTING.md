# Contributing to ep_padlist2

Thanks for helping improve `ep_padlist2` — a plugin for [Etherpad](https://etherpad.org/). LLM/Agent contributions are explicitly welcome, provided they follow the rules below.
...
`ep_padlist2` is a plugin, so you develop it inside a checkout of [ether/etherpad](https://github.com/ether/etherpad).
...
# Install ep_padlist2 from your local clone
pnpm run plugins i --path ../ep_padlist2

# Start the dev server (port 9001)
pnpm --filter ep_etherpad-lite run dev
```

Edits in `../ep_padlist2/` are picked up after a server restart.
...
* Etherpad version, ep_padlist2 version, browser + OS
* Steps to reproduce
